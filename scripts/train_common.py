...
class TrainingPipelineSpec:
    model_builder: Callable[[Dict[str, Any], Any, torch.device], nn.Module]
    export_artifact_fn: Callable[[nn.Module, Any], object]
    report_title: str
    quant_section_title: str
    quant_section_lines_fn: Callable[[Any], List[str]]
    export_result_key: str
    export_template: str
...
@torch.no_grad()
def eval_epoch(model, loader, criterion, device, window_sec: int, desc: str = "val") -> Dict[str, float]:
    model.eval()
    losses = []
    ys, ps = [], []
    pbar = tqdm(loader, desc=desc, leave=False)
...
def render_export_name(template: str, rank: int) -> str:
    try:
        return template.format(rank=rank)
    except KeyError as e:
        raise ValueError(f"Invalid export filename template: missing key {e}") from e


def run_nas_training_pipeline(args, spec: TrainingPipelineSpec) -> None:
    set_seed(args.seed)
    ...
    if args.dry_run:
        ...
        if ok:
            model = spec.model_builder(kwargs, args, device)
            dry["preflight"] = preflight_forward(model, train_loader, device=device)
        ...
        for arch in archs:
            ...
            try:
                ...
                for e in range(args.epochs):
                    ...
                    va = eval_epoch(model, val_loader, criterion, device, window_sec=window_sec, desc="val")
                ...
                val_m = eval_epoch(model, val_loader, criterion, device, window_sec=window_sec, desc="val-final")
                trial["status"] = "ok"
    ...
    for i, c in enumerate(best_two, start=1):
        model = c["model"]
        test_m = eval_epoch(model, test_loader, criterion, device, window_sec=window_sec, desc=f"test-rank-{i}")
        fp32_path = out_dir / f"best_{i}.pt"
        arch_path = out_dir / f"best_{i}_arch.json"
        export_name = render_export_name(spec.export_template, rank=i)
        export_path = out_dir / export_name

        torch.save(model.state_dict(), fp32_path)
        torch.save(spec.export_artifact_fn(model, args), export_path)
        arch_path.write_text(json.dumps(c["kwargs"], indent=2), encoding="utf-8")
        ...
        final_test_results.append(
            {
                ...
                spec.export_result_key: str(export_path),
            }
        )

    t_end = datetime.now()
    report_lines = []
    report_lines.append(spec.report_title)
    report_lines.append("")
    ...
    report_lines.append("")
    report_lines.append(spec.quant_section_title)
    report_lines.extend(spec.quant_section_lines_fn(args))
    report_lines.append("")
    ...
    for i, c in enumerate(best_two, start=1):
        vm = c["val_metrics"]
        report_lines.append(
            f"- rank_{i}: trial={c['trial_id']}, sensitivity={vm['sensitivity']:.6f}, far={vm['far']:.6f}, f1={vm['f1']:.6f}"
        )
    report_lines.append("")
    ...
    for r in final_test_results:
        tm = r["test_metrics"]
        report_lines.append(
            f"- rank_{r['rank']} trial={r['trial_id']}: sensitivity={tm['sensitivity']:.6f}, far={tm['far']:.6f}, f1={tm['f1']:.6f}, acc={tm['accuracy']:.6f}"
        )
        report_lines.append(f"  fp32_checkpoint: {r['fp32_checkpoint']}")
        report_lines.append(f"  {spec.export_result_key}: {r[spec.export_result_key]}")
